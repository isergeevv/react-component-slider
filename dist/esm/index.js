...
const parseConfig = (props) => {
    ...
    const type = (_a = props.type) !== null && _a !== void 0 ? _a : SLIDER_TYPE.HORIZONTAL;
    const direction = (_b = props.direction) !== null && _b !== void 0 ? _b : (type === SLIDER_TYPE.HORIZONTAL ? SLIDER_DIRECTIONS.RIGHT : SLIDER_DIRECTIONS.DOWN);
...
function useSlider(props) {
    ...
    const config = useRef(parseConfig(props !== null && props !== void 0 ? props : {}));
    const sliderContainerRef = useRef(null);
    const sliderRef = useRef(null);
    const offset = useRef(0);
    const lastTimestamp = useRef(0);
    const updateSliderInterval = useRef(null);
    ...
    const moveCallback = useCallback((timestamp) => {
        if (!playing)
            return;
        if (!sliderRef.current) {
            throw new Error('Missing items list element.');
...
export { SLIDER_DIRECTIONS, SLIDER_TYPE, useSlider as default };
